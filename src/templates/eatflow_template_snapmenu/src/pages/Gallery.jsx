...
// Default gallery items in case the context doesn't provide any
...
export function Gallery() {
  ...
  // Use gallery items from siteContent if available, otherwise use defaults
  const galleryItems = siteContent?.gallery?.images?.length > 0
    ? siteContent.gallery.images.map(item => ({
        category: item.description.includes("chef") ? "Behind the Scenes" :
                 item.description.includes("event") ? "Events" :
                 item.description.includes("dining") ? "Restaurant" : "Food",
        image: item.image,
        title: item.title,
  ...
  const categories = uniqueCategories.length > 1 ? uniqueCategories : defaultCategories;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative h-[70vh]">
        <div className="absolute inset-0">
          <img 
            src={siteContent?.hero?.banners?.[0]?.image || "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&q=80"}
        ...
        <div className="relative z-10 container mx-auto px-6 h-[calc(70vh-120px)] flex items-center justify-center text-center">
          <div>
            ...
            <h1 className="text-7xl font-bold text-white mb-8">
              {siteContent?.gallery?.section?.title || "Our Gallery"}
            </h1>
            <p className="text-2xl text-gray-200 max-w-3xl mx-auto leading-relaxed">
              {siteContent?.gallery?.section?.subtitle || "Take a visual journey through our restaurant, events, and culinary creations"}
            </p>
          </div>
        </div>
      </div>
