    ...
    if (validateForm()) {
      try {
        ...
        const orderData = {
          ...
          customerEmail: email,
          customerPhone: phone,
          orderMethod: delivery ? 'delivery' : 'takeout',
          deliveryAddress: delivery ? document.getElementById('address')?.value || '' : '',
          items: cart.map(item => ({
            id: item.id,
            name: item.name,
            price: item.price,
          ...
          tax: (calculateTotal() * 0.08).toFixed(2),
          deliveryFee: delivery ? '5.00' : '0.00',
          total: calculateTotalWithDelivery().toFixed(2),
          estimatedTime,
        ...
        if (orderResponse.paymentInfo?.payment_link) {
          window.open(orderResponse.paymentInfo.payment_link, '_blank');
        } else {
          // Clear cart and navigate to confirmation with order details
          setOrderPlaced(true);
          clearCart();
          navigate('/confirmation', { state: { orderDetails: orderResponse } });
        }
      } catch (error) {
