...
export function Footer() {
  ...
  const footerContent = siteContent?.footer || {
    ...
    linkGroups: [],
    social: { links: [] },
    copyright: { text: '© 2025 Restaurant. All rights reserved.' }
  };
    ...
    <footer className="bg-zinc-900 py-16 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
          <div>
            <h3 className="text-2xl font-bold mb-4">{siteContent?.brand?.name || 'Restaurant'}</h3>
        ...
        <div className="border-t border-zinc-800 mt-12 pt-8 text-center text-gray-400">
          <p>{footerContent?.copyright?.text || '© 2025 Restaurant. All rights reserved.'}</p>
        </div>
      </div>
    </footer>
  );
