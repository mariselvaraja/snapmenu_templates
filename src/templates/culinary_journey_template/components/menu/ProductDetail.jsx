...
import { ArrowLeft, Leaf, AlertTriangle, ArrowRight } from 'lucide-react';
...
export function ProductDetail() {
  ...
  if (menu && menu.menu) {
    ...
    if (menu.menu[categoryId]) {
      console.log(`Looking for product with id ${itemId} in category ${categoryId}`);
      item = menu.menu[categoryId].find(item => item.id === itemId);
      console.log("Found product:", item);
    } 
    
    // If item not found in the specified category, search all categories
    if (!item) {
      console.log(`Item not found in category ${categoryId}, searching all categories`);
      for (const category in menu.menu) {
        if (Array.isArray(menu.menu[category])) {
          const foundItem = menu.menu[category].find(item => item.id === itemId);
          if (foundItem) {
            console.log(`Found item in category ${category}:`, foundItem);
            item = foundItem;
            break;
          }
        }
  ...
  if (item.pairings && item.pairings.length > 0 && menu && menu.menu) {
    // Flatten all menu items into a single array
    const allMenuItems = Object.values(menu.menu)
      .flat()
      .filter(menuItem => menuItem && typeof menuItem === 'object');
    
    // Find paired items by name
    item.pairings.forEach(pairingName => {
      const pairedItem = allMenuItems.find(menuItem => 
        menuItem.name && menuItem.name.toLowerCase() === pairingName.toLowerCase()
      );
