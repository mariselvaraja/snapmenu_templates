...
export default function Home() {
  ...
  // Efficient sections object for reordering
  const sections = {
    experience: (experienceCard.cards && experienceCard.cards.length > 0) && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {experienceCard.cards.map((card: ExperienceCardItem, index: number) => {
              const iconMap: Record<string, IconComponent> = {
                Wine: FaWineGlass, UtensilsCrossed: FaUtensils, Clock: FaClock, Coffee: FaCoffee,
                GlassMartini: FaGlassMartini, Hamburger: FaHamburger, Pizza: FaPizzaSlice,
                ConciergeBell: FaConciergeBell, Chair: FaChair, Fire: FaFire, BreadSlice: FaBreadSlice,
                Truck: FaTruck, Motorcycle: FaMotorcycle, ShoppingBag: FaShoppingBag,
                MapMarker: FaMapMarkerAlt, Calendar: FaCalendarAlt, Users: FaUsers, Star: FaStar,
                MoneyBill: FaMoneyBillWave, Mobile: FaMobileAlt, Car: FaCarSide, Shipping: FaShippingFast,
                Percent: FaPercent, UserTie: FaUserTie, Clipboard: FaClipboardList, Receipt: FaReceipt
              };
              const IconComponent = iconMap[card.icon] || FaUtensils;
    ...
    popularitems: popularItems.products && popularItems.products.length > 0 && (
                          ...
                          <div className="p-6">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {!siteConfiguration?.hidePriceInWebsite && !siteConfiguration?.hidePriceInHome && (
                                isPaymentAvilable && (
                                  menuItem.inventory_status === false ? (
                                    <button className="inline-flex items-center bg-gray-400 text-white px-3 py-1 rounded-full cursor-not-allowed text-sm" disabled>
                                      Out Of Stock
                                    </button>
                                  ) : (
                                    <button
    ...
    offers: offers && Array.isArray(offers) && offers.length > 0 && (
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-4xl font-bold mb-6 text-center">Special Offers</h2>
          <div className="grid gap-8">
            {offers.map((offer: any, index: number) => (
              <div key={index} className="bg-gray-50 p-6 rounded-lg">
                <p className="text-lg">{offer.content || "Special offer available!"}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  // Section ordering logic - if sectionOrder exists use it, otherwise use default previous order
  const sectionOrder = homepage?.sectionOrder?.length > 0 
    ? homepage.sectionOrder 
    : ['experience', 'popularitems', 'combos', 'offers'];
  ...
  return (
    <div>
      ...
      {/* DYNAMIC SECTIONS - Efficient reordering with hero always on top */}
      {sectionOrder.map((sectionType: string, index: number) => 
        sections[sectionType as keyof typeof sections] ? (
          <div key={`${sectionType}-${index}`}>
            {sections[sectionType as keyof typeof sections]}
          </div>
        ) : null
      )}
