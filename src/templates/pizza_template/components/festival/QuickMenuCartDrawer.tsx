...
import QuickBillComponent from './QuickBillComponent';

interface QuickMenuCartDrawerProps {
  onPlaceOrder?: (specialRequest: string) => void;
  displayName?: string;
  onOrderSuccess?: () => void;
}

const QuickMenuCartDrawer: React.FC<QuickMenuCartDrawerProps> = ({ onPlaceOrder, displayName = 'Festival Menu', onOrderSuccess }) => {
  ...
  return (
    <>
                        ...
                        <div className="flex-1">
                          ...
                          {item.selectedModifiers && item.selectedModifiers.length > 0 && (
                            <div className="mt-1 mb-2 text-xs text-gray-600">
                              ...
                              {/* Spice level with quantity controls */}
                              {item.selectedModifiers.flatMap((modifier: any) => 
                                modifier.name === "Spice Level" ? 
                                  modifier.options.map((option: any, index: number) => (
                                    <div 
                          ...
                          {/* Quantity controls for items without spice level */}
      ...
      {/* Quick Bill Component - Show after placing order */}
      {showQuickBill && (
        <QuickBillComponent
          onClose={() => {
            setShowQuickBill(false);
            // Close the cart drawer after bill closes
            dispatch(toggleDrawer(false));
          }}
          onOrderSuccess={() => {
            // Call the onOrderSuccess callback from FestivalMenu
            if (onOrderSuccess) {
              onOrderSuccess();
            }
          }}
...
export default QuickMenuCartDrawer;
