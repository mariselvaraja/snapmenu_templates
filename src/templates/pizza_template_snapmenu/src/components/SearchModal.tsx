...
const SearchModal: React.FC<SearchModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleItemSelect = (result: any) => {
    // Navigate to a product-specific URL without specifying the category
    // This allows the ProductDetails component to find the product in any category
    const path = `/product/${result.item.id}`;
    handleClose();
    navigate(path);
  };
  ...
  const renderHighlightedText = (text: string, searchQuery: string) => {
    ...
    return parts.map((part, i) => 
      part.toLowerCase() === searchQuery.toLowerCase() ? 
        <span key={i} className="bg-red-800 text-white">{part}</span> : part
    );
  };
